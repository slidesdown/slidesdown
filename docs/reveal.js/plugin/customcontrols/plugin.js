...
const initCustomControls = function (Reveal) {
  ...
  toggleButton.innerHTML = '<span id="collapse-customcontrols">' +
    collapseIcon + "</span>" + '<span id="expand-customcontrols">' +
    expandIcon + "</span>";
  ...
  for (var i = 0; i < config.controls.length; i++) {
    ...
    control.innerHTML = "<button " + (config.controls[i].title
      ? 'title="' + config.controls[i].title + '" '
      : "") +
      'onclick="' + config.controls[i].action + '">' + config.controls[i].icon +
      "</button>";
